...
#     students = TblStudents.objects.filter().values('id','reg_no','status','college_id','branch_id','admission_session','semester_id','year_id','course_type_id','first_name','middle_name','last_name','father_name','primary_contact_no','is_mobile_verified','is_registered','id_card_pin', 'latitude', 'longitude').order_by('-id')
    
#     paginator = Paginator(students, getConfigurationResult('page_limit'))
#     enrolled = TblStudents.objects.filter().order_by('-id').values()
#     try:
...
#         if student['branch_id'] != "":
#             course = TblCourseTypes.objects.get(id=student['course_type_id'])
#             student['course'] = course.course_type
        
#         if student['admission_session'] !="":
#             admissionsession = TblClCollegeSession.objects.filter(id=student['admission_session']).first()
#             student['admissionsession'] = admissionsession.session
...
#             student['semester'] = str(student['semester_id'])+" Sem"
#         else:
#             student['semester'] = str(student['year_id'])+" Year"
...
#     student_details = TblStudents.objects.raw(''' SELECT tbl_students.*,tbl_colleges.college_name,tbl_branch.branch,
#     tbl_cl_basic_details.gender,tbl_cl_basic_details.caste_category,tbl_cl_basic_details.income_category,tbl_cl_basic_details.dob,
#     tbl_cl_college_session.session,tbl_semester.sem_name,tbl_cl_section.section_name FROM tbl_students
#     LEFT JOIN tbl_colleges on tbl_colleges.id = tbl_students.college_id 
#     LEFT JOIN tbl_branch on tbl_branch.id = tbl_students.branch_id
#     LEFT JOIN tbl_cl_basic_details on tbl_cl_basic_details.student_id = tbl_students.id
#     LEFT join tbl_cl_college_session on tbl_cl_college_session.id=tbl_students.admission_session
#     LEFT join tbl_semester on tbl_semester.semester_id = tbl_students.semester_id
#     left join tbl_cl_section on tbl_cl_section.id = tbl_students.section_id
#     WHERE tbl_students.is_registered = 1
#     ORDER BY  tbl_students.id DESC LIMIT 1
#     ''')
#     if student_details:
#         # Correspondance Address
#         if student_details[0].state_id is not None:
#           student_details[0].state_name = getModelColumnById(TblStates, student_details[0].state_id, "state")
#         if student_details[0].country_id is not None:
#           student_details[0].country_name = getModelColumnById(TblCountry, student_details[0].country_id, "country_name")
#         if student_details[0].tehsil_id is not None:   
#           student_details[0].tehsil_name = getModelColumnById(TblNewTehsil, student_details[0].tehsil_id, "tehsil_name")
#         # Permanent Address
#         if student_details[0].per_state_id is not None:
#           student_details[0].per_state_name = getModelColumnById(TblStates, student_details[0].per_state_id, "state")
#         if student_details[0].per_country_id is not None:
#           student_details[0].per_country_name = getModelColumnById(TblCountry, student_details[0].per_country_id, "country_name")
#         if student_details[0].per_tehsil_id is not None:
#           student_details[0].per_tehsil_name = getModelColumnById(TblNewTehsil, student_details[0].per_tehsil_id, "tehsil_name")
...
#         elif student_details[0].college_id == 3:
#             context['college_base_url'] = "http://bip.sortstring.co.in/"
#     allcourse = TblCourseTypes.objects.filter().order_by('-course_type').values()
#     allSemester = TblSemester.objects.filter().order_by('-sem_name').values()
#     allCollegeSession = TblClCollegeSession.objects.filter().order_by('-id').values()
#     context['students']                 = students
#     context['total_pages']              = total_pages
#     context['totalStudent']             = len(enrolled)
#     context['course']                   = allcourse 
#     context['allSemester']              = allSemester 
#     context['allCollegeSession']        = allCollegeSession
...
#         if student_details[0].per_tehsil_id is not None:
#           student_details[0].per_tehsil_name = getModelColumnById(TblNewTehsil, student_details[0].per_tehsil_id, "tehsil_name")
#         if student_details[0].semester_id:
#             context['semester'] = str(student_details[0].semester_id)+" Sem"
#         if student_details[0].year_id:
#             context['year'] = str(student_details[0].year_id)+" Year"    
...
#     # allbranch = TblBranch.objects.filter().order_by('-branch').values()
#     allbranch = TblCourseTypes.objects.filter().order_by('-id').values()
#     allSemester = TblSemester.objects.filter().order_by('-sem_name').values()
#     allCollegeSession = TblClCollegeSession.objects.filter().order_by('-id').values()
#     context['students']                 = students
#     context['total_pages']              = total_pages
...
def addNewStudentBasicDetails(request):
    ...
    if request.method == "POST":
        try:
            ...
            try:
                basic = TblClBasicDetails.objects.get(student_id=last_user_id)
            ...
            if basic:
                ...
                if request.POST['Privilege_category']:
                    ...
                    basic.privilage_category_id = 0
                    basic.privilage_category    = None
...
def addStudentBiometricDetails(request):
    if request.method == "POST":
        ...
        if request.POST['year_sem_id']:
            # user.semester_id         = request.POST['year_sem_id']
            if getModelColumnById(TblBranch, request.POST['branch_id'], 'total_year'):
                user.year_id             = request.POST['year_sem_id']
                user.semester_id         = ''
            else:    
                user.semester_id         = request.POST['year_sem_id']
                user.year_id             = None
        ...
        context['document_lists']    = TblClDocuments.objects.filter(student_id=last_user_id, document_path__isnull=False).order_by('-id')
        context['document_list_count']  = TblClDocuments.objects.filter(student_id=last_user_id, document_path__isnull=False).count()
        context['last_user_id']      = last_user_id 
        context['college_id']        = user.college_id   
        context['rooms']             = rooms 
...
def addStudentDocuments(request):
    ...
    if request.method == "POST":
        try:
            last_user_id = request.POST['last_user_id']
            if TblClDocuments.objects.filter(ducument_number=request.POST['document_number']).exists():
                response['error']   = True
                response['message'] = "Document No. already exists."
                return JsonResponse(response)
            elif TblClDocuments.objects.filter(student_id=last_user_id, document_name=getModelColumnById(TblClDocumentTypes, request.POST['document_type'], 'document_name')).exists():
                response['error']   = True
                ...
                organization_id = getModelColumnById(TblStudents, last_user_id, "college_id")
                all_users = SpUsers.objects.filter(organization_id = organization_id).exclude(id=last_user_id).values_list("id", flat=True)
...
def studentDocumentList(request):
    ...
    context['last_user_id']         = last_user_id
    context['document_lists']       = TblClDocuments.objects.filter(student_id=last_user_id, document_path__isnull=False).order_by('-id') 
    context['document_list_count']  = document_list_count  = TblClDocuments.objects.filter(student_id=last_user_id, document_path__isnull=False).count()   
...
def addNewFile(request):
    if request.method == "POST":
        file_path   = getModelColumnById(TblClDocuments, request.POST['document_id'], 'document_path')
        ...
        document_file           = TblClDocuments.objects.get(id=request.POST['document_id'])
        document_file.document_path = None
        document_file.save()
        
        file                    = TblClFolderFiles()
        file.student_id         = request.POST['student_id']
        ...
        context['document_list_count']  = TblClDocuments.objects.filter(student_id=request.GET['student_id'], document_path__isnull=False).count()
        context['document_details']     = TblClDocuments.objects.get(id=request.GET['document_id'], document_path__isnull=False)
...
def editStudentBiometricDetails(request):
    ...
    context['college_id']        = user.college_id
    if user.semester_id:
        context['semester'] = user.semester_id
    else:
        context['semester'] = user.year_id
...
def QRGeneration(request):
    ...
    if TblStudents.objects.filter(reg_no = reg_no).exists() == False:
        response['message']         = "No Registration Number found 1"
    ...
    if TblStudents.objects.filter(reg_no = reg_no, is_id_card_generated = 1).exists() and id_counter == 0:
        ...
        response['message']         = "No Registration Number found 2"
        response['response_status'] = 404
        return JsonResponse(response)
...
def getMasterDetails(request):
    ...
    if type == 'room_list':
        ...
        context['folder_files'] = TblClFolderFiles.objects.filter(file_id=id, college_id=college_id, student_id=last_user_id, room_id=context['room_id'], almira_id=context['almira_id'], rack_id=context['rack_id'])    
...
def viewDocuments(request):
    all_institution              = SpOrganizations.objects.filter(status=1)
    context     = {}
    context['page_title'] = 'All Documents'
    context['all_institution']   = all_institution
    template = 'student/view-documents.html'
    return render(request, template, context)   
    
@login_required
def getMasterRecord(request):
    context     = {}
    id          = request.POST['id']
    college_id  = request.POST['college_id']
    type        = request.POST['type']
    ...
    if type == 'college_list':
        context['master']       = 'college_list'
        context['all_institution'] = SpOrganizations.objects.filter()
        context['college_id']   = ''
        context['master_name']  = master_name  
    elif type == 'room_list':
        context['master']       = 'room'
        ...
        context['college_id']   = college_id
        context['master_name']  = master_name
        context['college_name'] = getModelColumnById(SpOrganizations, college_id, 'organization_name')    
    elif type == 'room':
        context['master']       = 'almira'
        context['almiras']      = TblClAlmirah.objects.filter(room_id=id)
        context['college_id']   = college_id
        ...
        context['almira_id']    = getModelColumnById(TblClRack, id, 'almira_id') 
        context['almira_name']  = getModelColumnById(TblClRack, id, 'almira_name') 
        context['files']        = TblClFileFolder.objects.filter(college_id=college_id, room_id=context['room_id'], almira_id=context['almira_id'], rack_id=context['rack_id'])   
        ...
        context['rack_id']      = getModelColumnById(TblClFileFolder, id, 'rack_id')
        context['rack_name']    = getModelColumnById(TblClRack, context['rack_id'], 'rack')
        context['folder_files'] = TblClFolderFiles.objects.filter(file_id=id, college_id=college_id, room_id=context['room_id'], almira_id=context['almira_id'], rack_id=context['rack_id'])    
        context['college_name'] = getModelColumnById(SpOrganizations, college_id, 'organization_name')    
        context['master_name']  = ''   
    ...
    context['baseurl']           = baseurl
    template                     = 'student/master-documents-details.html'
    return render(request, template, context)
...
def getDocumentData(request):
    ...
    context['rack_id']      = getModelColumnById(TblClFileFolder, id, 'rack_id')
    context['rack_name']    = getModelColumnById(TblClRack, context['rack_id'], 'rack')
    context['files_data'] = TblClFolderFiles.objects.filter(file_id=id, college_id=college_id, room_id=context['room_id'], almira_id=context['almira_id'], rack_id=context['rack_id'])
    context['master_name']  = ''     
    baseurl = settings.BASE_URL
    
    context['baseurl']           = baseurl
    template                     = 'student/view-document-data.html'
    return render(request, template, context) 
    
# for ajax function of studentfilter ---new development  
@login_required
def filterStudentList(request):
    context = {}
    
    # page = request.GET.get('page')
    students = TblStudents.objects.filter().values('id','reg_no','status','college_id','branch_id','year_id','admission_session','semester_id','course_type_id','first_name','middle_name','last_name','father_name','primary_contact_no','is_mobile_verified','is_registered','id_card_pin', 'latitude', 'longitude')
  
    if request.GET['filter_branch'] !='':
       students = students.filter(course_type_id=request.GET['filter_branch'])
       
    if request.GET['filter_course'] !='':
        students = students.filter(branch_id=request.GET['filter_course']) 
        # totalyear    = TblBranch.objects.get(id=request.GET['filter_course'])
        # if year_sem.total_year:
        #    students = students.filter(year_id=request.GET['filter_semester'])    
        # for year_sem in year_sem:
        #     print(year_sem.total_year)
        # if(len(year_sem['total_year']) >0):
        #     students = students.filter(year_id=request.GET['filter_semester'])
    if request.GET['filter_course'] !='' and request.GET['filter_semester'] !='':
        try:
            totalyear    = TblBranch.objects.get(id=request.GET['filter_course'])
        except TblBranch.DoesNotExist:
            totalyear  =None
        if totalyear:
            if totalyear.total_year:
                students = students.filter(year_id=request.GET['filter_semester'])
            if totalyear.total_sem:  
                students = students.filter(semester_id=request.GET['filter_semester'])      
         
    if request.GET['filter_session'] !='':
        students = students.filter(admission_session=request.GET['filter_session'])
    ...
    for student in students:
        ...
        if student['branch_id'] != "":
            ...
            student['course'] = course.course_type
        if student['semester_id'] !="":
            semester = TblSemester.objects.get(id=student['semester_id'])
            student['semester'] = str(semester.id)+" Sem"
        if student['year_id'] !=None:
            semester = TblSemester.objects.get(id=student['year_id'])
            student['semester'] = str(semester.id)+" Year"
    ...
    context['page_title']               = "Manage Students"
    context['google_app_key']           = getConfigurationResult('google_app_key')
    template = 'student/filter-student-list.html'
    return render(request, template, context)
