        ...
        try:
            ...
            if product_container_id == '':
                ...
                if product.id :
                    ...
                    saveActivity('Product & Variant Management', 'Create Product', heading, activity, request.user.id, user_name, 'add.png', '1', 'web.png')
        ...
        try:
            if SpProducts.objects.filter(product_name=request.POST['product_name']).exclude(id=request.POST['product_id']).exists():
                ...
                if product.id :
                    ...
                    saveActivity('Product & Variant Management', 'Edit Product', heading, activity, request.user.id, user_name, 'add.png', '1', 'web.png')
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if product_variant.id:
                ...
                operational_users = SpUsers.objects.filter(Q(is_distributor=1) | Q(is_super_stockist=1)).values('id')
                if len(operational_users) :
                    for operational_user in operational_users :
                        ...
                        user_product_variant.sp_distributor  = request.POST['sp_distributor']
                        user_product_variant.sp_superstockist  = request.POST['sp_superstockist']
                        user_product_variant.sp_employee  = request.POST['sp_employee']

                        user_product_variant.container_mrp  = float(request.POST['mrp']) * float(request.POST['no_of_pouch'])
                        user_product_variant.container_sp_distributor  = float(request.POST['sp_distributor']) * float(request.POST['no_of_pouch'])
                        user_product_variant.container_sp_superstockist  = float(request.POST['sp_superstockist']) * float(request.POST['no_of_pouch'])
                        user_product_variant.container_sp_employee  = float(request.POST['sp_employee']) * float(request.POST['no_of_pouch'])
                ...
                saveActivity('Product & Variant Management', 'Add Product Variant', heading, activity, request.user.id, user_name, 'add.png', '1', 'web.png')
        ...
        try:
            ...
            if (int(request.POST['update_confirm']) == 0) and ((float(request.POST['mrp']) !=  float(product_variant.mrp)) or (float(request.POST['sp_distributor']) !=  float(product_variant.sp_distributor)) or (float(request.POST['sp_superstockist']) !=  float(product_variant.sp_superstockist))) :
                ...
                if product_variant.id:
                    ...
                    SpUserProductVariants.objects.filter(product_variant_id=product_variant.id).update(
                        product_id = request.POST['product_id'],
                        product_name  = getModelColumnById(SpProducts,request.POST['product_id'],'product_name'),
                        product_variant_id = product_variant.id,
                        product_class_id  = getModelColumnById(SpProducts,request.POST['product_id'],'product_class_id'),
                        item_sku_code = request.POST['item_code'],
                        variant_quantity = request.POST['variant_qty'],
                        variant_unit_id  = request.POST['variant_unit'],
                        variant_name  = request.POST['variant_name'],
                        variant_unit_name  = getModelColumnById(SpProductUnits,request.POST['variant_unit'],'unit'),
                        largest_unit_name = getModelColumnById(SpProductUnits,request.POST['variant_unit'],'largest_unit'),
                        variant_size  = request.POST['variant_size'],
                        no_of_pouch  = request.POST['no_of_pouch'],
                        container_size  = request.POST['container_size'],
                        is_bulk_pack  = request.POST['variant_type'],
                        mrp  = request.POST['mrp'],
                        sp_distributor  = request.POST['sp_distributor'],
                        sp_superstockist  = request.POST['sp_superstockist'],
                        sp_employee  = request.POST['sp_employee'],
                        container_mrp  = float(request.POST['mrp']) * float(request.POST['no_of_pouch']),
                        container_sp_distributor  = float(request.POST['sp_distributor']) * float(request.POST['no_of_pouch']),
                        container_sp_superstockist  = float(request.POST['sp_superstockist']) * float(request.POST['no_of_pouch']),
                        container_sp_employee  = float(request.POST['sp_employee']) * float(request.POST['no_of_pouch'])
                        )
                    ...
                    saveActivity('Product & Variant Management', 'Edit Product Variant', heading, activity, request.user.id, user_name, 'add.png', '1', 'web.png')
...
def updateProductVariantStatus(request):
    ...
    if request.method == "POST":
        try:
            ...
            saveActivity('Product & Variant Management', 'Product Variant', heading, activity, request.user.id, user_name, 'add.png', '1', 'web.png')
