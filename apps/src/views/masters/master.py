...
def addHalfDaySlot(request):
    if request.method == "POST":
        id = request.POST['id']
        obj = TblClWorkingShifts.objects.get(id=id)
        obj.first_half_start_timing = request.POST['f_start_time_data']
        obj.first_half_end_timing = request.POST['f_end_time_data']
        obj.second_half_start_timing = request.POST['s_start_time_data']
        obj.second_half_end_timing = request.POST['s_end_time_data']
        obj.save()
        context = {}
        context['flag'] = True
        context['message'] = "Record updated successfully."
        ...
        context = {}
        id = request.GET['id']
        context['half_day_slot'] = TblClWorkingShifts.objects.get(id=id)
        template = 'master/attendance-group/add-hlaf-day-slot.html'
        return render(request, template, context)
        
#  BMC .................

def ajaxBMCList(request):
    context = {}
    bmc_list = SpBMC.objects.all().values()
   
   
    context['bmc'] = bmc_list
    template = 'master/bmc/ajax-bmc.html'
    return render(request, template, context)

@login_required
def addBMCList(request):
    if request.method == "POST":
        response = {}
        try:
            if SpBMC.objects.filter(bmc_code=request.POST['bmc_code']).exists():
                response['flag'] = False
                response['message'] = "BMC already exists."
            else:
                addBmc = SpBMC()
                addBmc.name = request.POST['name']
                addBmc.bmc_code = request.POST['bmc_code']
                addBmc.save()
                response['flag'] = True
                response['message'] = "Record has been saved successfully."
        except:
        ...
        template = 'master/bmc/add-bmc.html'
        return render(request, template, context)


@login_required
def editBMCList(request, bmc_id):
    if request.method == "POST":
        response = {}
        if SpBMC.objects.filter(bmc_code=request.POST['bmc_code']).exclude(id=bmc_id).exists():
            response['flag'] = False
            response['message'] = "BMC already exists."
        else:
            try:
                
                editBMC = SpBMC.objects.get(id=bmc_id)
                editBMC.name = request.POST['name']
                editBMC.bmc_code = request.POST['bmc_code']
        ...
        context = {}
        context['bmc'] = SpBMC.objects.get(id=bmc_id)
        
        template = 'master/bmc/edit-bmc.html'
        return render(request, template, context)


@login_required
def updateBMCStatus(request,bmc_id):
    context = {}
    if SpBMC.objects.filter(id=bmc_id).exists():
        try:
            bmc_status = SpBMC.objects.get(id=bmc_id)
        
            if int(bmc_status.status) == 1:
                bmc_status.status = 0
                bmc_status.save()
            else:
                bmc_status.status = 1
...
#  MPP .................
@login_required
def ajaxMPPList(request):
    context = {}
    mpp_list = SpMPP.objects.all().values()
    for mpp in mpp_list:
        if mpp['bmc_id']:
            bmc_object = SpBMC.objects.filter(id = mpp['bmc_id']).first()
            mpp['bmc_name'] = bmc_object.name
        if mpp['employee_id']:
            
            mpp['employee_name'] = getUserName(mpp['employee_id'])
    context['mpp'] =  mpp_list
    template = 'master/mpp/ajax-mpp.html'
    return render(request, template, context)
...
@login_required
def addMPPList(request):
    if request.method == "POST":
        response = {}
        try:
            if SpMPP.objects.filter(mpp_code=request.POST['mpp_code']).exists():
                response['flag'] = False
                response['message'] = "MPP already exists."
            else:
                get_bmc_code = getModelColumnById(SpBMC,request.POST['bmc_id'],'bmc_code')
                get_mpp_tr_code = genrateMPPTrCode(get_bmc_code, request.POST['mpp_code'],)
                addMpp = SpMPP()
                addMpp.name = request.POST['name']
                addMpp.mpp_code = request.POST['mpp_code']
                if request.POST['employee_id']:
                    addMpp.employee_id =  request.POST['employee_id']
                if request.POST['bmc_id']:
                    addMpp.bmc_id =  request.POST['bmc_id']
                if request.POST['latitude']:
                    addMpp.latitude =  request.POST['latitude']
                if request.POST['longitude']:
                    addMpp.longitude =  request.POST['longitude']
                if get_mpp_tr_code:
                    addMpp.mpp_tr_code = get_mpp_tr_code
                
                if request.POST['wef_date']:
                    wef_date = datetime.strptime(request.POST['wef_date'], '%d/%m/%Y')
                    addMpp.wef_date = wef_date
                addMpp.save()
        ...
        context = {}
        context['reporting_user_list'] = SpUsers.objects.all()
        context['bmc_list'] = SpBMC.objects.all()
        template = 'master/mpp/add-mpp.html'
        return render(request, template, context)


@login_required
def editMPPList(request, mpp_id):
   
    if request.method == "POST":
        response = {}
        
        if SpMPP.objects.filter(mpp_code=request.POST['mpp_code']).exclude(id=int(mpp_id)).exists():
            response['flag'] = False
            response['message'] = "MPP already exists."
        else:
            try:
                get_bmc_code = getModelColumnById(SpBMC,request.POST['bmc_id'],'bmc_code')
                get_mpp_tr_code = genrateMPPTrCode(get_bmc_code, request.POST['mpp_code'],)
                mpp_code = request.POST['mpp_code']
                editMPP = SpMPP.objects.get(id=mpp_id)
                editMPP.name = request.POST['name']
                editMPP.mpp_code = request.POST['mpp_code']
                if request.POST['employee_id']:
                    editMPP.employee_id =  request.POST['employee_id']
                if request.POST['bmc_id']:
                    editMPP.bmc_id =  request.POST['bmc_id']
                if request.POST['latitude']:
                    editMPP.latitude =  request.POST['latitude']
                if request.POST['longitude']:
                    editMPP.longitude =  request.POST['longitude']
                
                editMPP.mpp_tr_code = get_mpp_tr_code
                if request.POST['wef_date']:
                    wef_date = datetime.strptime(request.POST['wef_date'], '%d/%m/%Y')
                    editMPP.wef_date = wef_date
                editMPP.save()
                response['flag'] = True
                response['message'] = "Record has been updated successfully."
            except:
        ...
        context = {}
        context['mpp'] = SpMPP.objects.get(id=mpp_id)
        context['bmc_list'] = SpBMC.objects.all()
        context['reporting_user_list'] = SpUsers.objects.all()
        template = 'master/mpp/edit-mpp.html'
        return render(request, template, context)


@login_required
def updateMPPStatus(request,mpp_id):
    context = {}
    if SpMPP.objects.filter(id=mpp_id).exists():
        try:
            mpp_status = SpMPP.objects.get(id=mpp_id)
            if int(mpp_status.status) == 1:
                mpp_status.status = 0
                mpp_status.save()
            else:
                mpp_status.status = 1
...
# def exportMpp(request):
#     response = HttpResponse(content_type='text/csv')
#     response['Content-Disposition'] = 'attachment; filename="mpp_data.csv"'

#     writer = csv.writer(response)

#     # Write SpMPP headers
#     writer.writerow(['MPP ID', 'Name', 'MPP Code', 'BMC Name', 'Employee First Name', 'MPP TR Code', 'Wef Date', 'Latitude', 'Longitude', 'Status'])

#     # Query all SpMPP records
#     for mpp in SpMPP.objects.all():
#         bmc_name = mpp.bmc.name if mpp.bmc else ''
#         employee_first_name = mpp.employee.first_name +' '+ mpp.employee.last_name if mpp.employee else ''
#         bmc_code =  mpp.bmc.bmc_code if mpp.bmc else ''
#         writer.writerow([
#             mpp.id,  # Or any unique identifier for MPP
#             mpp.name,
#             mpp.mpp_code,
#             bmc_code,
#             bmc_name,
...
def exportMpp(request):
    ...
    # Define header data
    headers = ['MPP ID', 'Name', 'MPP Code', 'BMC CODE', 'BMC Name', 'Employee Code', 'Employee First Name', 'MPP TR Code', 'Wef Date', 'Latitude', 'Longitude', 'Status']
    ...
    # Query all SpMPP records
    for row_num, mpp in enumerate(SpMPP.objects.all(), 2):
        worksheet.cell(row=row_num, column=1, value=mpp.id)
        worksheet.cell(row=row_num, column=2, value=mpp.name)
        worksheet.cell(row=row_num, column=3, value=mpp.mpp_code)
        worksheet.cell(row=row_num, column=4, value=mpp.bmc.bmc_code if mpp.bmc else '')
        worksheet.cell(row=row_num, column=5, value=mpp.bmc.name if mpp.bmc else '')
        worksheet.cell(row=row_num, column=6, value=mpp.employee.emp_sap_id if mpp.employee else '')
        worksheet.cell(row=row_num, column=7, value=mpp.employee.first_name +' '+ mpp.employee.last_name if mpp.employee else '')
        worksheet.cell(row=row_num, column=8, value=mpp.mpp_tr_code)
        worksheet.cell(row=row_num, column=9, value=mpp.wef_date)
        worksheet.cell(row=row_num, column=10, value=mpp.latitude)
        worksheet.cell(row=row_num, column=11, value=mpp.longitude)
        worksheet.cell(row=row_num, column=12, value=mpp.status)
    ...
    response['Content-Disposition'] = 'attachment; filename="mpp_data.xlsx"'
