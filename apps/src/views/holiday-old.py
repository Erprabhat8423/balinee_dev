    ...
    if request.method == "POST":
        ...
        if request.POST['filter_org_name[]']:
            ...
            for org_id in request.POST.getlist('filter_org_name[]'):
                ...
                semester = request.POST.getlist('sem_name[]')
                if branch[0] == "all":
                    b2 = TblBranch.objects.filter(college_id = org_id)
                    for b2 in b2:
                        branch_1.append(b2.id)
                    applicable_to['branch'] = branch_1
                else:
                    for branch in branch:
                        b1 = TblBranch.objects.filter(id = branch).filter(college_id = org_id)
                        for b1 in b1:
                            if int(b1.college_id) == int(org_id):
                                branch_1.append(branch)
                        applicable_to['branch'] = branch_1

                for each_role in role:
                    r1 = SpRoles.objects.filter(id = int(each_role)).filter(organization_id = org_id)
                    if r1:
                        applicable_to['role'] = each_role
                        break
                    else:
                        applicable_to['role'] = '' 

                if semester[0] == "all":
                    s1 = TblSemester.objects.filter(type__startswith="semester")
                    for s1 in s1:
                        semester_1.append(s1.semester_id)
                    applicable_to['semester'] = semester_1
                else:
                    applicable_to['semester']  = request.POST.getlist('sem_name[]')
                ...
                holiday.organization_name = org_name
                holiday.organization_id = org_id
                holiday.applicable_to=applicable_to
                holiday.start_time = "00:00:00" 
                holiday.start_date = datetime.strptime(clean_data(request.POST['from_date']), '%d/%m/%Y').strftime('%Y-%m-%d')
                holiday.end_date = datetime.strptime(clean_data(request.POST['to_date']), '%d/%m/%Y').strftime('%Y-%m-%d')
                holiday.end_time = "00:00:00"
...
def editHoliday(request,holiday_id):
    if request.method == "POST":
        ...
        if SpHolidays.objects.filter(holiday=holiday_name,organization_id=filter_org_name).exclude(id=holiday_id).exists():
            ...
            semester = request.POST.getlist('sem_name[]')
            if branch[0] == "all":
                b2 = TblBranch.objects.filter(college_id = filter_org_name)
                for b2 in b2:
                    branch_1.append(b2.id)
                applicable_to['branch'] = branch_1
            else:
                for branch in branch:
                    b1 = TblBranch.objects.filter(id = branch).filter(college_id = filter_org_name)
                    for b1 in b1:
                        if int(b1.college_id) == int(filter_org_name):
                            branch_1.append(branch)
                    applicable_to['branch'] = branch_1

            for each_role in role:
                r1 = SpRoles.objects.filter(id = int(each_role)).filter(organization_id = filter_org_name)
                if r1:
                    applicable_to['role'] = each_role
                    break
                else:
        ...
        organiztion_id = SpHolidays.objects.get(id=holiday_id)
        context['role_mappings'] = SpRoleEntityMapping.objects.filter(entity_type='holiday',entity_id=holiday_id).values_list('role_id',flat=True)
        context['holiday'] = holiday = SpHolidays.objects.get(id=holiday_id)
        context['applicable_to'] = holiday.applicable_to
        context['holiday_org'] = holiday.organization_id
        context['roles'] = SpRoles.objects.filter(role_name__regex = r'%s'%'[s|S]tudent')
        context['holiday_types'] = SpHolidayTypes.objects.filter(status=1)
        context['semester']=TblSemester.objects.filter(type__startswith="semester")
        context['branch']= TblBranch.objects.filter(college_id = organiztion_id.organization_id)

        print(holiday.applicable_to['branch'], context['branch'])
        template = 'holiday/edit-holiday.html'
